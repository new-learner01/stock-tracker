...
def get_stock_metrics(cand_symbol):
  try:
    ...
    ret_1y = (
        float(
            ((cur_p - float(prices.iloc[0])) / float(prices.iloc[0])) * 100
        )
        if len(prices) > 200
        else 0.0
    )
    ...
    return {
        ...
        'roce': (
            round(inf.get('returnOnCapital', 15.0), 2)
            if inf.get('returnOnCapital')
            else 15.0
        ),
        'roe': (
            round(inf.get('returnOnEquity', 0.14) * 100, 2)
            if inf.get('returnOnEquity')
            else 14.0
        ),
        'de': (
            round(inf.get('debtToEquity', 80.0) / 100, 2)
            if inf.get('debtToEquity')
            else 0.8
        ),
  ...
  for path in [
    ...
    if os.path.exists(path):
      with open(path, 'r', encoding='utf-8') as f:
        return f.read()
  return (
      '<h1>index.html not found. Please create index.html in your repository'
      ' root.</h1>',
      404,
  )
  ...
  try:
    ...
    if is_multiyear_breakout:
      ...
      pattern_analysis = (
          ...
          f' (₹{dma200:.2f}). Accumulation zone on dips.'
      )
      is_bullish = True
    else:
    ...
    stock2_metrics = (
        get_stock_metrics(
            f'{peer1_sym}.NS'
            if peer1_sym and not peer1_sym.endswith(('.NS', '.BO'))
            else peer1_sym
        )
        if peer1_sym
        else None
    ...
    stock3_metrics = (
        get_stock_metrics(
            f'{peer2_sym}.NS'
            if peer2_sym and not peer2_sym.endswith(('.NS', '.BO'))
            else peer2_sym
        )
        if peer2_sym
        else None
