...
<div class="container">
  <div class="card search-bar">
    <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
      ...
      <input type="text" id="peer1" class="search-input" placeholder="Compare Stock 2 (Optional)" style="width:170px;" value="">
      <input type="text" id="peer2" class="search-input" placeholder="Compare Stock 3 (Optional)" style="width:170px;" value="">
      ...
      <table class="brokerage-table" id="comparison-table">
        <thead>
          <tr>
            ...
            <th id="th-stock2" style="color:var(--purple); display:none;">Comparison Stock 2</th>
            <th id="th-stock3" style="color:var(--green); display:none;">Comparison Stock 3</th>
          </tr>
        </thead>
        <tbody id="comparison-table-body">
          <tr><td colspan="4" style="text-align:center; color:var(--muted);">Loading fundamentals...</td></tr>
        </tbody>
      </table>
    </div>
  </div>
...
async function loadStock() {
  ...
  if (!sym) {
    showStatus("❌ Please enter a primary stock ticker.", "notice-error");
    return;
  }
...
def get_stock():
    try:
        ...
        if pcr_val >= 1.20:
            ...
            oi_bullish = True
            oi_interp = f"Put-Call Ratio at {pcr_val:.2f} reflects aggressive put writing at ₹{max_put_strike:.2f}, creating a solid price floor."
        elif pcr_val <= 0.75:
            oi_signal = "🔴 Bearish (Call Writing Dominance)"
            oi_pred = "DOWNWARD / Capped Upside"
            oi_bullish = False
            oi_interp = f"Low PCR of {pcr_val:.2f} indicates massive call buildup at ₹{max_call_strike:.2f}. Stiff overhead resistance expected."
        else:
            oi_signal = "🟡 Neutral / Rangebound"
            oi_pred = "SIDEWAYS (Consolidation)"
            oi_bullish = True
            oi_interp = f"Balanced PCR of {pcr_val:.2f}. Open interest is concentrated between support and resistance."
        ...
        stock2_metrics = get_stock_metrics(f"{peer1_sym}.NS" if peer1_sym and not peer1_sym.endswith(('.NS', '.BO')) else peer1_sym) if peer1_sym else None
        stock3_metrics = get_stock_metrics(f"{peer2_sym}.NS" if peer2_sym and not peer2_sym.endswith(('.NS', '.BO')) else peer2_sym) if peer2_sym else None
