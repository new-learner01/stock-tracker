...
<title>Pro Stock Terminal & Screener Analytics</title>
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
  ...
  .search-input { background: #080c14; border: 1px solid var(--border); color: #fff; padding: 10px 16px; border-radius: 8px; font-size: 0.95rem; width: 240px; text-transform: uppercase; font-weight: 700; outline: none; }
...
<div class="container">
  ...
  <div class="card search-bar">
    <div style="display:flex; gap:8px; align-items:center;">
      <input type="text" id="ticker" class="search-input" placeholder="Enter Ticker (e.g. KEC)" value="KEC" onkeydown="if(event.key==='Enter') loadStock()">
  ...
  <div class="card header-banner">
    <div>
      ...
      <p id="sector-info" style="color:var(--cyan); font-size:0.78rem; margin-top:2px;">-</p>
    </div>
...
async function loadStock() {
  ...
  if (!sym) {
    showStatus("❌ Please enter a stock ticker.", "notice-error");
    return;
  }

  showStatus(`Running live queries for ${sym}...`, "notice-loading");
  ...
  try {
    ...
    document.getElementById('sector-info').innerText = `Sector: ${data.sector} • Industry: ${data.industry}`;
...
def get_stock():
  try:
    ...
    if is_multiyear_breakout:
      ...
      pattern_analysis = (
          f'Stock is surging near its multi-year / all-time high of ₹{high_5y:.2f}.'
          ' Price discovery expansion with institutional accumulation across'
      ...
      pattern_analysis = (
          f'Price is testing and breaching the 52-week horizontal resistance at'
          f' ₹{high_52w:.2f}. Higher-low consolidation indicates imminent trend'
          ' expansion.'
      )
      is_bullish = True
      ...
      pattern_analysis = (
          ...
          f' (₹{dma200:.2f}). Pullbacks towards 20 DMA offer favorable'
          ' accumulation entry.'
      )
      is_bullish = True
      ...
      pattern_analysis = (
          'Stock is trading below its 200 DMA in a stage-1 base building'
          ' structure. Reversal requires a sustained close above the 50 DMA'
          ' resistance.'
      )
      is_bullish = False
    ...
    try:
      if ticker_obj and hasattr(ticker_obj, 'options') and ticker_obj.options:
    ...
    if pcr_val >= 1.20:
      ...
      oi_bullish = True
      oi_interp = (
          f'Put-Call Ratio at {pcr_val:.2f} reflects aggressive put writing at'
          f' ₹{max_put_strike:.2f}, creating a solid price floor.'
      )
    elif pcr_val <= 0.75:
      ...
      oi_bullish = False
      oi_interp = (
          f'Low PCR of {pcr_val:.2f} indicates massive call buildup at'
          f' ₹{max_call_strike:.2f}. Stiff overhead resistance expected.'
      )
    else:
      ...
      oi_bullish = True
      oi_interp = (
          f'Balanced PCR of {pcr_val:.2f}. Open interest is concentrated'
    ...
    payload = {
        ...
        'pe': float(pe) if pe and not np.isnan(pe) else None,
        ...
        'currentLiab': (
            float(currentLiab) if currentLiab and not np.isnan(currentLiab) else 0.0
        ),
